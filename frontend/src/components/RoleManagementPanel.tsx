...
interface RoleManagementPanelProps {
    ...
    /** MODERATOR+ only, enforced server-side too - same threshold as onUpdateStatus. */
    onUpdateMediaPermission: (targetUserId: string, mediaAllowed: boolean) => void;
}
...
export function RoleManagementPanel({
    ...
    return (
        <div className="divide-y divide-line">
            ...
            <span className="text-sm text-ink/80 flex-1 truncate inline-flex items-center gap-2">
              ...
              {member.status === 'ACTIVE' ? 'مجاز' : member.status === 'RESTRICTED' ? 'محدود' : 'مسدود'}
                        ...
                        {/* same permission tier as the block/restrict buttons below (MODERATOR+, outranks target). */}
                        {statusEditable && (
                            <button
                                onClick={() => onUpdateMediaPermission(member.userId, !member.mediaAllowed)}
                                title={member.mediaAllowed ? 'محدود کردن ارسال رسانه' : 'اجازه ارسال رسانه'}
                                className={`p-1.5 rounded-lg border transition-colors ${
                                    member.mediaAllowed
                                        ? 'border-line text-ink/50 hover:border-red-200 hover:text-red-600 hover:bg-red-50'
                                        : 'border-yellow-200 text-yellow-700 bg-yellow-50'
                                }`}
                            >
                                {member.mediaAllowed ? <Image className="w-3.5 h-3.5" /> : <ImageOff className="w-3.5 h-3.5" />}
                            </button>
                        )}
